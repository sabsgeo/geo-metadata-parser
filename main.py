...
def add_series_metadata(number_of_process):
    diff_list = general_helper.get_diff_between_all_geo_series_and_series_metadata()
    parallel_runner.add_data_in_parallel(__add_series_metadata_to_mongo, diff_list, number_of_process, 250, True)
...
def sync_metadata_status_from_geo(number_of_process):
    ...
    parallel_runner.add_data_in_parallel(__add_geo_sync_info_to_mongo, series_pattern, number_of_process, 10, False)


def main(function_call, process_number):
...
if __name__ == "__main__":
    ...
    main(args.function, int(args.process))
